...
class AStarNode:
    ...
    def get_neighbors(self, occupied_msg: dict, pad_list: list, space_cost_graph: CostGraph,
        ...
        if x < grid_size[0] - 1 and direct != [-1, 0, 0]:
            ...
            if str(pos) not in self.end_pos_set:
                if space_cost_graph.is_extended(pos):
                    ...
                    track_space_cost = calculate_space(pos, clearance_with_track, occupied_msg)
                    via_space_cost = calculate_space(pos, clearance_with_microvia, occupied_msg)
                    space_cost_graph.extend_grid(pos, track_space_cost, via_space_cost)
                    space_cost = track_space_cost
                g_cost += space_cost

            item = AStarNode(pos, self.start_pos, self.end_pos, self.end_pos_set, self.g_score, g_cost + 1, self)
        ...
        if x < grid_size[0] - 1 and y < grid_size[1] - 1 and direct != [-1, -1, 0]:
            ...
            item = AStarNode(pos, self.start_pos, self.end_pos, self.end_pos_set, self.g_score, g_cost + 1.414, self)
            neighbors.append(item)
        # go to the north
...
class AStarRouter:
    ...
    def route_two_pin_net(self, start: list, net_pin_set: set, end: list, end_set: set, net_id: int) -> (list, float):
        ...
        for pos in net_pin_set:
            ...
            start_item = AStarNode(start_pos, start, end, end_set, 0.0, 0.0, None)
